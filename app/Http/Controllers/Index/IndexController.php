...
{
    // PC端
    public function pcalipay($order_id)
    {
        // require_once dirname(dirname(__FILE__)).'/config.php';
        $config = config('pay');
        // dd($config);exit;
         // dd(require_once app_path('libs\pcalipay\pagepay\buildermodel\AlipayTradePagePayContentBuilder.php'));
        require_once app_path('libs\pcalipay\pagepay\service\AlipayTradeService.php');
        require_once app_path('libs\pcalipay\pagepay\buildermodel\AlipayTradePagePayContentBuilder.php');
        // $order_id = request()->order_id;
        $res  = DB::table('order')->where('order_id','=',$order_id)->first();
        // dd($res);   
    ...
    public function returnpay()
    {
        ...
        */
        if($result) {//验证成功
            ...
            dd($res);
            if($res){
                        echo "支付宝交易号：".$trade_no."订单号：".$where['order_no']."订单金额：".$where['order_amount']."此订单有问题";exit;
                    }

                    if(config('pay.seller_id')!=htmlspecialchars($_GET['seller_id'])){
                        echo "支付宝交易号：".$trade_no."订单号：".$where['order_no']."订单金额：".$where['order_amount']."此订单有问题,商户id不匹配";exit;

                    }
                    if(config('pay.app_id')!=htmlspecialchars($_GET['app_id'])){
                        echo "支付宝交易号：".$trade_no."订单号：".$where['order_no']."订单金额：".$where['order_amount']."此订单有问题,appid不匹配";exit;
    ...
    public function phonealipay($order_id)
    {
        // echo ok;exit;
        $config = config('pay');
        // dd($config);
        require_once app_path('libs/phonealipay/wappay/service/AlipayTradeService.php');
        require_once app_path('libs/phonealipay/wappay/buildermodel/AlipayTradeWapPayContentBuilder.php');
        // dd();
         // echo 123;exit;
